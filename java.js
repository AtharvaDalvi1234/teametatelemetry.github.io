...
var dataRef8 = database.ref('GYRO');
...
dataRef7.on('value', function(getdata7){
  ...
  document.getElementById('speed').innerHTML = _speed + " kmph" ;
...
// Define resetValues function in the global scope
window.resetValues = function() {
  ...
  database.ref('GYRO').set(defaultValues, (error) => {
    if (error) {
  ...
  database.ref('GYRO/calibrate').set(true, (error) => {
    if (error) {
      console.log("Data could not be saved." + error);
    } else {
      console.log("Calibration initiated successfully.");
    }
  });
}
